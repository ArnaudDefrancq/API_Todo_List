...
export class DAO<T> {
    ...
    public create(item: T, callback: (error: Error | null, insertId?: number) => void): void {
        DB.init();
        const db = DB.getDb();
        const queryString: string = `INSERT INTO ${this.tableName} SET ?`;
    ...
    public find(where: string, select: string = '*', queryString?: string): Promise<T[]> {
        DB.init();
        const db = DB.getDb();
        const query: string = queryString || `SELECT ${select} FROM ${this.tableName} ${where}`;        
    ...
    public findById(id: number, select: string = '*', queryString?: string): Promise<T[]> {
        DB.init();
        const db = DB.getDb();
        const query: string = queryString || `SELECT ${select} FROM ${this.tableName} WHERE id_${this.tableName} = ?`;
    ...
    public update(id: number, item: Partial<T>, callback: (error: Error | null, affectedRows?: number) => void): void {
        DB.init();
        const db = DB.getDb();
        const queryString: string = `UPDATE ${this.tableName} SET ? WHERE id_${this.tableName} = ?`;
    ...
    public delete(id: number, callback: (error: Error | null, affectedRows?: number) => void): void {
        DB.init();
        const db = DB.getDb();
        const query: string = `DELETE FROM ${this.tableName} WHERE id_${this.tableName} = ?`;
